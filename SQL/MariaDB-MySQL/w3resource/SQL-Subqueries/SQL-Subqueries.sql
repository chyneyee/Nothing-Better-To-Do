/* 13-01-2023 11:13 am
 * Reference: https://www.w3resource.com/sql-exercises/subqueries/index.php
 * Subqueries
 */
...
WHERE 1 <
	(SELECT COUNT(*) FROM customer WHERE salesman_id = s.salesman_id);
...
WHERE purch_amt >
	(SELECT AVG(purch_amt) FROM orders b WHERE a.customer_id = b.customer_id);
...
WHERE 1 =
	(SELECT COUNT(customer_id) FROM customer c WHERE s.salesman_id = c.salesman_id);

/* 18. From the following tables, write a SQL query to find the salespeople who deal the customers with more than one order. 
Return salesman_id, name, city and commission. */
SELECT * FROM salesman s
WHERE EXISTS
	(SELECT * FROM customer c WHERE s.salesman_id = c.salesman_id
     AND 1 < 
     		(SELECT COUNT (*) FROM orders WHERE orders.customer_id = c.customer_id));
...
SELECT * FROM salesman
WHERE city IN
	(SELECT city FROM customer WHERE salesman.city = customer.city);
...
WHERE EXISTS
   (SELECT * FROM CUSTOMER b WHERE  a.name  < b.cust_name);
...
SELECT * FROM orders
WHERE purch_amt < ANY
	(SELECT purch_amt FROM orders o, customer c WHERE o.customer_id = c.customer_id
     AND c.city = 'London');
...
SELECT * FROM orders
WHERE purch_amt <
	(SELECT MAX(purch_amt) FROM orders o, custoemr c WHERE o.customer_id = c.customer.id
     AND c.city = 'London');
...
WHERE grade > ALL
	(SELECT grade FROM customer b WHERE city = 'New York');

/* 27. From the following tables write a SQL query to calculate the total order amount generated by a salesperson. Salespersons should be from the cities where the customers reside. 
...
Return Average Price and Company. */
SELECT AVG(i.pro_price) AS "Average Price", c.com_name AS "Company" FROM item_mast i, company_mast c
WHERE i.pro_com = c.com_id
GROUP BY c.com_name
...
WHERE dpt_code IN
	(SELECT emp_dept FROM emp_details GROUP BY emp_dept HAVING COUN(*) > 2);
